	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (devices.isEmpty()) {
			...
			if (devices.size() == 1) {
				...
				String registrationId = devices.get(0).getGcm_regid();
				Builder msgBuilder = new Message.Builder();
				msgBuilder.addData("ticket_type", "talk");
				msgBuilder.addData("ticket_content", "challenge");
				...
				int counter = 0;
				int tasks = 0;
				for (UserProfile device : devices) {
					counter++;
					partialDevices.add(device.getGcm_regid());
